...
async function main() {
  ...
  const panagoraOrg = await prisma.organization.create({
    data: {
      name: 'Panagora Marketing',
      slug: 'panagora-marketing',
      type: 'CLIENT',
    },
  });
  ...
  await prisma.user.create({
    data: {
      ...
      name: 'Panagora Client',
      role: 'CLIENT_VIEWER',
      organizationId: panagoraOrg.id,
    },
  });
  ...
  // Clients & Brands
  // ========================================
  const panagora = await prisma.client.create({
    data: {
      name: 'Panagora Marketing',
      slug: 'panagora-marketing',
      primaryColor: '#E11D48',
  ...
  const chantecler = await prisma.brand.create({
    data: {
      name: 'Chantecler',
      slug: 'chantecler',
      clientId: panagora.id,
    },
  });
  ...
  const study = await prisma.study.create({
    data: {
      name: 'Mauritius Poultry Brand Tracker 2023-2024',
      description: 'Quarterly consumer brand health tracking study for poultry brands in Mauritius',
      brandId: chantecler.id,
      country: 'Mauritius',
  ...
  // Competitor Brands
  // ========================================
  const competitors = ['Avipro', 'Chantefrais', 'Label 60', 'Marilyn'];
  ...
  // ========================================
  // Brand Metrics — Chantecler (realistic trend data)
  // ========================================
  const chanteclerMetrics: Record<string, number[]> = {
  ...
  for (const [metric, values] of Object.entries(chanteclerMetrics)) {
  ...
  console.warn('  admin@trackfield.com / TrackField@2024');
  console.warn('  analyst@trackfield.com / Analyst@2024');
  console.warn('  panagora@client.mu / Client@2024');
}
